...
class YamlChecker(object):
    ...
    def check(self, paths):
        ...
        yaml_conf = YamlLintConfig(file=os.path.join(
            os.path.dirname(ansible_test.__file__),
            'lib/sanity/yamllint/config/default.yml')
        )
        #module_conf = YamlLintConfig(file='test/sanity/yamllint/config/modules.yml')
        module_conf = YamlLintConfig(file=os.path.join(
            os.path.dirname(ansible_test.__file__),
            'lib/sanity/yamllint/config/modules.yml')
        )
        #plugin_conf = YamlLintConfig(file='test/sanity/yamllint/config/plugins.yml')
        plugin_conf = YamlLintConfig(file=os.path.join(
            os.path.dirname(ansible_test.__file__),
            'lib/sanity/yamllint/config/plugins.yml')
        )
    ...
    def get_module_docs(self, path, contents):
        ...
        if path.startswith('plugins/modules/') or path.startswith('lib/ansible/plugins/'):
        ...
        elif path.startswith('plugins/doc_fragments/'):
            for body_statement in module_ast.body:

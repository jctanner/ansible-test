...
class AnsibleDocTest(SanityMultipleVersion):
    ...
    def test(self, args, targets, python_version):
        ...
        plugins = [x for x in targets.include if x.path.startswith('plugins')]
        plugins = [x.path.split('/') for x in plugins]
        plugins = [[x[1], x[2], '/'.join(x)] for x in plugins]
        for idx,x in enumerate(plugins):
            if x[0] == 'modules':
                plugins[idx][0] = 'module'

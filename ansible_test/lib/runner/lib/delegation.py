...
def build_src_tar(tarfilen, tar_file):
    '''Pack the ansible_test distribution'''

    atdir = os.path.dirname(ansible_test.__file__)

    '''
    filenames = list(Path(atdir).rglob('*'))
    filenames = [x for x in filenames if not x.parts[-1].endswith('.pyc')]
    filenames = [x for x in filenames if not x.parts[-1].endswith('.pyo')]
    filenames = [x for x in filenames if not '__pycache__' in x.parts]

    with tarfile.TarFile.gzopen(tarfilen, mode='w', compresslevel=4) as tar:
        for filename in filenames:
            if not filename.is_file():
                continue
            src = str(filename)
            dst = src.replace(os.path.dirname(atdir) + '/', '')
            tar.addfile(tarfile.TarInfo(dst), open(filename, 'rb').read())
    '''

    with tarfile.TarFile.gzopen(tarfilen, mode='w', compresslevel=4) as tar:
        tar.add(atdir, arcname='.')
...
def delegate_docker(args, exclude, require, integration_targets):
    ...
    cmd = generate_command(args, python_interpreter, '/root/ansible/ansible_test/bin/ansible-test-container-bin', options, exclude, require)
    cmd = cmd[3:]
    ...
    with tempfile.NamedTemporaryFile(prefix='ansible-source-', suffix='.tgz') as local_source_fd:
        try:
            ...
            docker_put(args, test_id, local_source_fd.name, '/root/ansible_test.tgz')
            docker_exec(args, test_id, ['mkdir', '-p', '/root/ansible/ansible_test'])
            docker_exec(args, test_id, ['tar', 'oxzf', '/root/ansible_test.tgz', '-C', '/root/ansible/ansible_test'])

            # fixup the ansible-test installation
            docker_exec(args, test_id, ['mkdir', '-p', '/root/ansible/test/results'])
            docker_exec(args, test_id, ['mkdir', '-p', '/root/ansible/test/test'])
            docker_exec(args, test_id, ['chmod', '+x', '/root/ansible/ansible_test/bin/ansible-test'])
            docker_exec(args, test_id, ['chmod', '+x', '/root/ansible/ansible_test/bin/ansible-test-container-bin'])

            # add the metadata file
            docker_put(args, test_id, args.metadata_path, '/root/ansible/test/%s' % os.path.basename(args.metadata_path))
            midx = cmd.index(os.path.basename(args.metadata_path))
            cmd[midx] = '/root/ansible/test/%s' % os.path.basename(args.metadata_path)
            ...
            try:
                ...
                with tempfile.NamedTemporaryFile(prefix='ansible-result-', suffix='.tgz') as local_result_fd:
                    docker_exec(args, test_id, ['tar', 'czf', '/root/results.tgz', '-C', '/root/ansible/test', 'results', 'test'])

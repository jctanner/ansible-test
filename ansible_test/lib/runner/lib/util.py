...
def get_parameterized_completion(cache, name):
    ...
    if not cache:
        ...
        images = read_lines_without_comments(pattern, remove_blank_lines=True)
...
def run_command(args, cmd, capture=False, env=None, data=None, cwd=None, always=False, stdin=None, stdout=None,
                cmd_verbosity=1, str_errors='strict', extra_env=None):
    """
    ...
    return raw_command(cmd, capture=capture, env=env, data=data, cwd=cwd, explain=explain, stdin=stdin, stdout=stdout,
                       cmd_verbosity=cmd_verbosity, str_errors=str_errors, extra_env=extra_env)
